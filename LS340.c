...
{	  
    ...
    if(!strcmp(acqchan->channel->label, ls->channels[ONEK]->channel->label)) {sens_name = "B"; alarm = 1;} 
    if(!strcmp(acqchan->channel->label, ls->channels[HE3P]->channel->label)) sens_name = "C"; 
...
void LS340_UpdateReadings(int panel, void *ptr)
{
	...
	GetHeaterLvl(ls->source->acqchan);
    SetCtrlVal(panel, LS340_CTRL_SORBTSET, ls->source->acqchan->reading);
}
...
void LS340_Init(void)
{
	...
	if(utilG.acq.status != ACQ_NONE)
	{
		...
		if(devType)
		{
			Fmt(devType->label, "ls 340 temperature controller");

...
void port_Save (portPtr port)
{
	if(port->type == ANALOGUE)
	{
		if(port->direction == IN_PORT)
		{
			...
			FmtFile(fileHandle.analysis, "%i, %i\n", 	port->port.analogueIOport.channel, 
	...
	if(dev)
	{
		if(port->type == ANALOGUE)
		{
			...
			{										 
				...
				ScanFile(fileHandle.analysis, "%i, %i[x]",	&port->port.analogueIOport.channel, 
															&port->port.analogueIOport.range);
		...
		if(port->type == DIGITAL)
		{
			if(port->direction == IN_PORT)
			{
				...
				ScanFile(fileHandle.analysis, "%i, %i, %i[x]",	&port->port.digitalIOport.devtype,
																&port->port.digitalIOport.port,
																&port->port.digitalIOport.bits);

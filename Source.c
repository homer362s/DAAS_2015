...
int  RemoveSourceCallback(int panel, int control, int event, void *callbackData, int eventData1, int eventData2)
{
	...
	if (event == EVENT_COMMIT)
	{
		sourcelist_RemoveSource (exp_GetSourceSelection());
    ...
    switch (control) {
        ...
        case SOURCE_OUTPUT:
            if (event == EVENT_COMMIT) {
				...
				SetCtrlVal (panel, control, src->biaslevel);
				//(src->acqchan->reading / src->acqchan->coeff)
            }
            break;
        case SOURCE_LABEL:
            if (event == EVENT_COMMIT) {
...
static int segment_Endof (segmentPtr seg)
{
    if ((seg->pt + 1) < seg->points) return FALSE;
    else return TRUE;
}
static double segment_Start(segmentPtr seg, int range)
{

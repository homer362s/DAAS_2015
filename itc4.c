...
typedef struct{
	...
	struct{int on; double setp;} heater1;
}itc4Type;
...
int itc4ControlCallback (int panel, int control, int event, void *callbackData, int eventData1, int eventData2)
{
	switch (control)
	{
		case ITC4_CTRL_HEATER1:
			if(event == EVENT_COMMIT)
			{
				rs232Ptr dev = callbackData;
				itc4Ptr  itc = dev->device;
				GetCtrlVal(panel, control, &itc->heater1.on);
				FlushOutQ(dev->COM->port);
				rs232Write (dev, "H1\r");
				rs232Write (dev, "[A1\r]", itc->heater1.on);
				rs232Write (dev, "[T%f\r]", itc->heater1.on, &itc->heater1.setp);
				rs232Write (dev, "[A0\r]", !itc->heater1.on);
				rs232Write (dev, "[O0\r]", !itc->heater1.on);
			}
			break;
		case ITC4_CTRL_SETP:
			if(event == EVENT_COMMIT)
			{
				rs232Ptr dev = callbackData;
				itc4Ptr  itc = dev->device;
				GetCtrlVal(panel, control, &itc->heater1.setp);
				FlushOutQ(dev->COM->port);
				rs232Write (dev, "[H1\r]", itc->heater1.on);
				rs232Write (dev, "[[T%f\r]]", itc->heater1.on, (int)itc->heater1.setp, &itc->heater1.setp);
			}
			break;
	}
	return 0;
...
void itc4Operate(int menubar, int menuItem, void *callbackData, int panel)
{
	...
	if(!dev->iPanel)
	{
		...
		SetCtrlAttribute(dev->iPanel, ITC4_CTRL_HEATER1, ATTR_CALLBACK_DATA, dev);
		SetCtrlAttribute(dev->iPanel, ITC4_CTRL_SETP,	 ATTR_CALLBACK_DATA, dev);
...
void itc4_Save(rs232Ptr dev)
{
	...
	FmtFile(fileHandle.analysis, "Heater1   :%i, %f\n", itc->heater1.on, itc->heater1.setp);
...
void itc4_Load(rs232Ptr dev)
{
	...
	if(itc)
	{
		ScanFile(fileHandle.analysis, "Heater1   :%i, %f\n", &itc->heater1.on, &itc->heater1.setp);
